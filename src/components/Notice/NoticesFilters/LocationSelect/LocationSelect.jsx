...
export default function LocationSelect({ value, onChange }) {
  ...
  const handleChangeLocation = (e) => {
    ...
    // Обновляем inputValue при выборе элемента
    if (e) {
      const selectedOption = options?.find(
        (option) => option.value === e.value
      );
      if (selectedOption) {
        setInputValue(selectedOption.label.split(", ")[1]); // Получаем cityEn из label
      }
    }
  ...
  return (
    <>
      <Select
        onInputChange={(value) => setInputValue(value)} // Используем непосредственно setInputValue
        onChange={handleChangeLocation}
        value={selectValue}
        options={options}
        placeholder={"Location"}
