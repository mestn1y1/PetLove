...
export default function RadioBtnSort({ field, form, onChange }) {
  const handleClick = (e) => {
    ...
    form.setFieldValue(field.name, chosenValue); // Обновляем значение в Formik
    onChange(chosenValue); // Вызываем onChange, переданный через пропсы
  };

  const handleCancel = (e) => {
    e.stopPropagation();
    form.setFieldValue(field.name, null); // Сбрасываем значение в Formik
    onChange(null); // Вызываем onChange, переданный через пропсы
  };
